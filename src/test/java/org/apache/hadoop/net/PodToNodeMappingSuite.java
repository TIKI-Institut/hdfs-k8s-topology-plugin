...
public class PodToNodeMappingSuite {
    ...
    public static void runUnitTest() {
        ...
        PodList expectedPodList = new PodListBuilder().withItems(
                new PodBuilder().withNewMetadata().withName("pod1").endMetadata().withNewStatus().withPodIP(podIPs[0]).endStatus().withNewSpec().withNodeName("kubernetes-worker-1").endSpec().build(),
                new PodBuilder().withNewMetadata().withName("pod2").endMetadata().withNewStatus().withPodIP(podIPs[1]).endStatus().withNewSpec().withNodeName("kubernetes-worker-3").endSpec().build()).build();
    ...
    public static String netString(String s){return PodToNodeMapping.RACK_NAME + "/" + s;}

    public static void main(String[] args) {
        PodToNodeMappingSuite.runUnitTest();
    }
